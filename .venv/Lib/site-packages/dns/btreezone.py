...
class WritableVersion(dns.zone.WritableVersion):
    def __init__(self, zone: dns.zone.Zone, replacement: bool = False):
        ...
        if not replacement:
            ...
            self.nodes: dns.btree.BTreeDict[dns.name.Name, Node] = dns.btree.BTreeDict(
                original=version.nodes
            )
            self.delegations = Delegations(original=version.delegations)
        else:
    ...
    def put_rdataset(
        ...
        if (
            rdataset.rdtype == dns.rdatatype.NS and not node.is_origin_or_glue()
        ):
            node.flags |= NodeFlags.DELEGATION
    ...
    def delete_rdataset(
        ...
        (node, name) = self._maybe_cow_with_name(name)
        if rdtype == dns.rdatatype.NS and name in self.delegations:
            node.flags &= ~NodeFlags.DELEGATION
            self.delegations.discard(name)
            self.update_glue_flag(name, False)
